...
class Employer(User):
    ...
    def accept_not_attend_meeting(self, request_id):
        # First, we should verify that the user making the request belongs to the same team
        check_query = '''
        SELECT mar.UserID
        FROM MeetingAttendanceRequests mar
        JOIN Users u ON mar.UserID = u.UserID
        WHERE mar.RequestID = %s AND u.Team = %s
        '''
        update_query = '''
        UPDATE MeetingAttendanceRequests
        SET Status = 'Αποδεκτή'
        WHERE RequestID = %s
        '''
        with get_cursor() as cursor:
            cursor.execute(check_query, (request_id, self.team_id))
            user_team = cursor.fetchone()
            if user_team:  # If the user is in the same team
    ...
    def deny_not_attend_meeting(self, request_id):
        # First, we should verify that the user making the request belongs to the same team
        check_query = '''
        SELECT mar.UserID
        FROM MeetingAttendanceRequests mar
...
if __name__ == "__main__":
    ...
    if employers:
        ...
        user_id_to_accept_not_attend = 1  # Example user ID for which the withdrawal request is to be accepted
        employer.deny_not_attend_meeting(user_id_to_accept_not_attend)
